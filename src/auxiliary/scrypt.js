...
function bytesToHex(bytes) {
    /** @const */
    let enc = '0123456789abcdef'.split('');
    ...
    for (; i < len; i++) {
        arr.push(enc[(bytes[i] >>> 4) & 15]);
        arr.push(enc[(bytes[i] >>> 0) & 15]);
    }
    return arr.join('');
}
...
async function scrypt(password, salt, N, r, p, dkLen) {
    ...
    function PBKDF2_HMAC_SHA256_OneIter(password, salt, dkLen) {
        ...
        let i, innerLen = 64 + salt.length + 4,
    ...
    let tmp = new Uint32Array(16);

    let B = PBKDF2_HMAC_SHA256_OneIter(stringToUTF8Bytes(password), stringToUTF8Bytes(salt), p * 128 * r);
